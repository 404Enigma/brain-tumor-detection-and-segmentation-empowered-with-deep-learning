...
class Pix2Pix(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, device, learning_rate=0.0002, save_after=5, LOAD_MODEL=None):
        # LOAD_MODEL : str, should contain "/" at end
        super().__init__()
    ...
    def init_params(self, in_channels, out_channels, learning_rate, device, LOAD_MODEL):
        # initialization of models ,optimizers etc
    ...
    def init_logs(self, save_after):
        # initilization of all logging variables
        self.VAL_LOSS = []
        self.GAN_LOSS = []
        self.DIS_LOSS = []
        self.GAN_LOSS_VAL = []
        ...
        # checkpoint created after every 10 epochs
    ...
    def load_model(self, LOAD_MODEL):
        print("Loading Model ...")
        LOAD_MODEL = sorted(list(next(os.walk(self.save_path))[
                            2]), key=lambda x: int(x[5:-4]))
        ...
        for epoch in (range(epochs)):
            ...
            try:
                for data in tqdm(train_loader):
                    ...
                    fake_target = torch.autograd.Variable(
                        torch.zeros(MR.size(0), 1, 8, 8).to(self.device))
                    real_target = torch.autograd.Variable(
                        torch.ones(MR.size(0), 1, 8, 8).to(self.device))
                    if PRINTLOG:
                        print(
                ...
                for data in tqdm(val_loader):
                    ...
                    fake_target = torch.autograd.Variable(
                        torch.zeros(MR.size(0), 1, 8, 8).to(device))
                    real_target = torch.autograd.Variable(
                        torch.ones(MR.size(0), 1, 8, 8).to(device))
                ...
                #print('\nImg shape ',img.shape,MR.shape,CT.shape)
